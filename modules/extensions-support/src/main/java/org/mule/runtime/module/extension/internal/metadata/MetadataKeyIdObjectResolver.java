...
final class MetadataKeyIdObjectResolver {
  ...
  private Optional<QueryModelProperty> getQueryModelProperty(ComponentModel componentModel) {
    return componentModel.getParameterModels().stream()
        .filter(p -> p.getModelProperty(QueryModelProperty.class).isPresent())
        .map(p -> p.getModelProperty(QueryModelProperty.class).get())
        .findAny();
  }
}
